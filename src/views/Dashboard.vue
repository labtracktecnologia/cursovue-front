        ...
        <b-card header="Traffic &amp; Sales">
          ...
          <b-table class="mb-0 table-outline" responsive="sm" hover :items="tableItems" :fields="tableFields" head-variant="light">
            <template v-slot:cell(avatar)="{ item }">
              <img width="40" :src="item.avatar.url" class="img-avatar" alt="">
              <span class="avatar-status" v-bind:class="{ 'bg-success': item.avatar.status == 'success',  'bg-warning': item.avatar.status == 'warning', 'bg-danger': item.avatar.status == 'danger', 'bg-secondary': item.avatar.status == '' }"></span>
            </template>
            ...
            <template v-slot:cell(country)="{ item }">
              <i class="h4 mb-0" :class="flag(item.country.flag)" :title="item.country.flag" :id="item.country.flag"></i>
            </template>
            <template v-slot:cell(usage)="{ item }">
              ...
              <b-progress class="progress-xs" v-model="item.usage.value" :variant="variant(item.usage.value)"></b-progress>
            </template>
...
export default {
  ...
  data: function () {
    return {
      ...
      tableFields: [
       { key: 'avatar', labelHtml: '<i class="icon-people"></i>', class: 'text-center' },
       { key: 'user', label: 'User' },
       { key: 'country', label: 'Country', class: 'text-center' },
       { key: 'usage', label: 'Usage' },
       { key: 'payment', label: 'Payment method', class: 'text-center' },
